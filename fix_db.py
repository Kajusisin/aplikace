...
def fix_database():
    ...
    try:
        ...
        # 2. Kontrola sloupce 'skolni_rok' v tabulce 'reference_scores'
        cursor.execute("PRAGMA table_info(reference_scores)")
        columns = cursor.fetchall()
        column_names = [col[1] for col in columns]
        
        if 'skolni_rok' not in column_names:
            print("➕ Přidávám sloupec 'skolni_rok' do tabulky 'reference_scores'...")
            cursor.execute("ALTER TABLE reference_scores ADD COLUMN skolni_rok INTEGER")
            ...
            cursor.execute(f"UPDATE reference_scores SET skolni_rok = {skolni_rok}")
            conn.commit()
